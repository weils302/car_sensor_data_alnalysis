...
# 車両基本情報のCSS selector
CSS_BODY_PRICE = 'body > div.page > div:nth-child(5) > main > section > div > div.column__sub > div.priceWrap > div.basePrice > p.basePrice__price'
CSS_TOTAL_PRICE = 'body > div.page > div:nth-child(5) > main > section > div > div.column__sub > div.priceWrap > div.totalPrice > p.totalPrice__price'
...
URL = 'https://www.carsensor.net/usedcar/search.php?CARC=BM_S011'
...
def main():
    ...
    for i in range(1, total_list_num + 1):
        ...
        df = collect_insert(urls, df)  # Pass the DataFrame to the function
...
def collect_insert(urls, df):
    for url in urls:
        key = extract_key(url)
        if key not in df['key'].values:
            print('add ', key)
            usedcar = scrape_detail_page(url, key)
            df = df.append(usedcar, ignore_index=True)  # Append the data to the DataFrame
    return df
